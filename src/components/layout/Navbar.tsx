...
type MenuItem = { label: string; href: string; highlight?: boolean; };
type MenuColumn = { heading?: string; items: MenuItem[]; };
type MenuPromo = { href: string; image: string; title: string; subtitle: string; };
type MegaMenu = { col1?: MenuColumn; col2?: MenuColumn; col3?: MenuColumn; promo?: MenuPromo; };
...
export default function Navbar() {
  ...
  const [mobileVisible, setMobileVisible] = useState(false); // controls CSS animation
  ...
  // Slide-in animation: mount first, then trigger visible state
  const openMobile = () => {
    setMobileOpen(true);
  ...
  const closeMobile = () => {
    ...
    setTimeout(() => setMobileOpen(false), 300); // wait for slide-out
  };
  ...
  return (
    <>
      ...
      {/* Announcement bar */}
      <div className="bg-[#F5F5F3] md:bg-white text-[#000] font-semibold text-[11px] p-2">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10 flex items-center justify-between">
          <span>
            New to Losode?{" "}
            <Link href="/products" className="underline hover:text-[#C8A96E]">Subscribe</Link>{" "}
      ...
      {/* Header */}
      <header className="bg-white md:bg-[#000] sticky top-0 z-50">
          ...
          <div className="border-b border-gray-200 md:border-white/10">
            <div className="max-w-[1400px] mx-auto px-6 lg:px-10 flex items-center justify-between h-16 md:h-[72px]">
              ...
              <div className="flex items-center gap-4 flex-1">
                <button onClick={openMobile} className="text-[#1A1A1A] md:hidden" aria-label="Menu">
              ...
              <Link href="/" className="absolute left-1/2 -translate-x-1/2">
                <Image src="/losode-mob.png" alt="Losode" width={130} height={44} priority className="block md:hidden object-contain h-9 w-auto" />
                <Image src="/og-image.png" alt="Losode" width={180} height={54} priority className="hidden md:block object-contain h-[54px] w-auto" />
              </Link>
              ...
              <div className="flex items-center gap-4 md:gap-6 flex-1 justify-end">
                ...
                <button className="hidden md:flex items-center justify-center text-white hover:text-[#C8A96E] transition-colors" aria-label="Account">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                ...
                <Link href="/favorites" className="hidden md:flex items-center justify-center text-white hover:text-[#C8A96E] transition-colors" aria-label="Wishlist">
                  <Badge count={favCount} size="small" color="#C8A96E" offset={[4, -4]}>
                ...
                {/* Cart */}
                <Link href="/cart" className="flex items-center justify-center text-[#1A1A1A] md:text-white hover:text-[#C8A96E] transition-colors" aria-label="Cart">
              ...
              <nav className="flex items-center h-11">
                ...
                {navbarData.categories.map(c => (
                  <div
                    ...
                    onMouseEnter={() => {
                      if (!c.noDropdown) setActiveMenu(c.label as MegaKey);
        ...
        {/* Mega menu — attached directly to hovered item's column, no global onMouseLeave */}
        {activeMenu && menu && (
          <div
                      ...
                      {menu.col1.items.map(item => (
                        <li key={item.label}>
                          <Link href={item.href} onClick={() => setActiveMenu(null)}
                            className={`text-[13px] leading-snug transition-colors hover:text-[#C8A96E] ${item.highlight ? "text-red-600 font-medium" : "text-[#1A1A1A]"}`}>
                            {item.label}
                          </Link>
                        </li>
                      ))}
                  ...
                  <div>
                    <Link href={menu.promo.href} onClick={() => setActiveMenu(null)}>
                      <div className="relative w-full aspect-[4/3] overflow-hidden bg-[#f0e8d8]">
                        <Image src={menu.promo.image} alt={menu.promo.title} fill className="object-cover hover:scale-105 transition-transform duration-500" />
                      </div>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-[150] lg:hidden">
          <div
            className={`mobile-overlay absolute inset-0 bg-black/60 backdrop-blur-sm ${mobileVisible ? "is-open" : ""}`}
            onClick={closeMobile}
          />
          <div className={`mobile-drawer absolute left-0 top-0 bottom-0 w-[290px] bg-white flex flex-col shadow-2xl ${mobileVisible ? "is-open" : ""}`}>

            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
              <Image src="/losode-mob.png" alt="Losode" width={110} height={36} className="object-contain h-8 w-auto" />
              <button onClick={closeMobile} className="text-gray-400 hover:text-gray-800 text-lg leading-none">✕</button>
            </div>
